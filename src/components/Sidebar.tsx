...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange, onLogout, logoUrl, userRole, customPermissions }) => {
  const menuConfig = [
    ...
    { id: ViewType.EBD, label: 'Escola Bíblica - EBD', icon: 'M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5S19.832 5.477 21 6.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253', roles: ['Admin', 'Secretaria', 'Membro'] },
    ...
    { id: ViewType.CAMPO_MISSIONARIO, label: 'DP Missões', icon: 'M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z', roles: ['Admin', 'Secretaria'] },
  ...
  return (
    <aside className="hidden md:flex flex-col h-full w-72 bg-slate-900 border-r border-slate-800 z-50 transition-all shadow-2xl overflow-hidden shrink-0">
      <div className="p-8 flex flex-col items-center border-b border-white/5 bg-slate-900/50">
        <div className="w-16 h-16 mb-4 bg-white rounded-2xl flex items-center justify-center p-2 shadow-xl overflow-hidden border border-slate-100">
          <img
            ...
            onError={(e: React.SyntheticEvent<HTMLImageElement, Event>) => { e.currentTarget.src = "https://ui-avatars.com/api/?name=Canaa&background=ffffff&color=ef4444"; }}
          />
        </div>
        <h1 className="text-[11px] font-heading font-black text-white tracking-[0.3em] text-center uppercase leading-tight">
          AD MISSIONÁRIA <br />
          <span className="text-primary tracking-[0.4em]">CANAÃ</span>
        </h1>
      </div>
      ...
      <div className="p-6 border-t border-white/5">
        <button
          onClick={onLogout}
          className="w-full flex items-center px-5 py-4 text-[10px] font-black text-slate-500 hover:text-primary transition-colors uppercase tracking-widest"
        >
